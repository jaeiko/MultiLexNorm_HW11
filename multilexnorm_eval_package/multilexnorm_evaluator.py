...
def command_evaluate(args: argparse.Namespace) -> None:
    ...
    validate_parallel(raw, gold, pred, langs)
    result = evaluate_all(raw, gold, pred, langs, model_name=model_name, dataset_name=args.dataset_name, notes=args.notes)

    out_dir = Path(args.out_dir)
    stem = f"{args.dataset_name}_{model_name}".replace(" ", "_")
    ...
    print(json.dumps(summarize_overall_result(result), ensure_ascii=False, indent=2))
    print(f"Saved evaluation to: {out_dir.resolve()}")
