...
This version extends the earlier common prompt with rules needed for Turkish
and Turkish-German code-switched text:
- target-only editing
- protected-token handling
- code-switching: normalize surface form, do not translate across languages
- proper noun / suffix / clitic caution
"""
...
NUMBER_DATE_TIME_RE = re.compile(r"^[0-9]+(?:[.:/\-][0-9]+)*(?:['’][A-Za-zÇĞİÖŞÜçğıöşü]+)?$|^[0-9]+(?:st|nd|rd|th)$", re.IGNORECASE)
...
- Do not translate Turkish words into German or German words into Turkish.
- If the TARGET language is unclear, preserve unless the local context strongly supports a minimal surface correction.

Proper noun / suffix / clitic rule:
- Proper names and entities should be preserved unless the annotation style clearly supports a minimal surface correction such as capitalization or Turkish apostrophe restoration.
- If the TARGET may require splitting, clitic separation, or apostrophe insertion, return only the normalized form for the TARGET token/span. Do not rewrite neighboring tokens.
""".strip()
